...
**Post-Stage-8 review fixes** are recorded at the end under "Post-review fixes" â€” these address findings from the parallel general/security code review run after Stage 8 wrap-up.

---

## Stage 2 â€” `generate_token`: CSPRNG nonce instead of timestamp
...
1. **Concurrent-request safety.** Two requests for the same URL inside the same second would, in the reference, produce the *same* hash on attempt 0 â€” both burn through every retry on identical inputs until the loop gives up. `secrets.token_bytes` draws from the OS CSPRNG, so each attempt is independent regardless of clock or process.
...
- The reference is marginally easier to reason about during a live whiteboard ("what's the input?" â†’ "URL plus the time"). Ours requires the reader to know `secrets`.
...
## Stage 3 â€” `validate_url`: scoped normalization, no scheme upgrade
...
**Why we changed it â€” four behavioral differences:**
...
2. **Query string case is preserved.** Same reasoning â€” many APIs use case-sensitive query values (signatures, base64 payloads, OAuth tokens). The reference would break a signed S3 link.

3. **No `http â†’ https` upgrade.** Not every target supports TLS on 443. Forcing the upgrade means a fraction of redirects silently 404 or hang on a refused connection. Mainstream URL shorteners (bit.ly, tinyurl) don't do this. If the user submits `http://`, we honour their intent and let the downstream server decide whether to redirect them to HTTPS.
...
- Less aggressive deduplication. The reference would treat `HTTPS://Example.com/foo` and `https://example.com/Foo` as the same URL; we treat them as different. That's a feature, not a bug â€” but it's a feature with a cost in storage and analytics granularity.
- Doesn't help us actually dedupe by URL anyway, because `generate_token` injects a random nonce â€” same URL â†’ different token regardless. So the normalization here is for *display consistency* and *downstream string comparison*, not for collision avoidance.

---

## Stage 4 â€” `redirect`: TTL-aware cache, naive-UTC time, tz coercion at boundary
...
**Why we changed it â€” three behavioral fixes:**
...
2. **`datetime.utcnow()` â†’ `datetime.now(timezone.utc).replace(tzinfo=None)`** via a `_now_naive()` helper. `utcnow()` is deprecated in Python 3.12 and emits a `DeprecationWarning`. We keep the naive return so it stays comparable with the `DateTime` columns in `models.py` (which are declared without `timezone=True`).
...
**Minor tightening:** `< now` â†’ `<= now` for the expiry check. The reference treats the exact-expiry instant as still-valid; we treat it as gone. Either is defensible, but `<=` matches the semantics most people expect from "expires at 2pm" ("at 2:00:00.000 it's already over").

**Trade-offs:**

- Cache stores 24 extra bytes per entry (datetime object). At 1M cached tokens, that's ~24 MB â€” trivial.
...
- If we migrate to Redis, the TTL-aware logic becomes redundant â€” Redis's `EXPIRE` handles eviction natively. The `_to_naive_utc` and `_now_naive` helpers can also retire if we switch to `DateTime(timezone=True)` columns.
...
- POST create â†’ `t5fWiKb` token returned
- GET `/r/{token}` â†’ 302 to `https://example.com/Hello` (note: case-preserved path)
- GET `/r/NOPE123` â†’ 404
- PATCH new URL â†’ 200, `original_url` updated
- GET `/r/{token}` â†’ 302 to `https://new-target.com`
- DELETE â†’ 200
- GET `/r/{token}` â†’ 410 "Gone â€” this link has been deleted"

---

## Stage 5 â€” test layer + `models.py` datetime deprecation

The reference repo has no test suite â€” `answers/` is implementation only. We add two:

1. **`tests/test_api.py` (25 tests, pytest + FastAPI TestClient)** â€” covers all 8 PROMPT.md scenarios plus regressions specific to the Stage 2â€“4 deviations:
   - URL normalization preserves path/query case
   - No `http â†’ https` upgrade
   - Root-only trailing slash collapses only when no query/fragment follows
   - Past-`expires_at` link returns 410
   - tz-aware ISO with `Z` suffix doesn't crash redirect (this would actually fail against the reference's redirect handler â€” see Stage 4 deviation 3)
   - PATCH and DELETE both invalidate the cache

2. **`scripts/smoke.ps1`** â€” Windows-native PowerShell script that hits a *running* server with the 8 PROMPT scenarios + the tz-Z regression. Uses `Invoke-RestMethod` / `Invoke-WebRequest` to dodge the [PowerShell-5.1 native-arg quote-eating bug](https://github.com/PowerShell/PowerShell/issues/1995) that broke our first attempt at a `curl.exe`-based script. Exits non-zero on failure.
...
## Stage 6 â€” static HTML frontend (additive)
...
**Signature wart:** the slowapi decorator reads the client IP off a parameter literally named `request: Request`. We didn't otherwise use `request` in `create_qr` â€” but the decorator can't find the IP otherwise, so we add it with an explanatory comment. This is a known slowapi ergonomic cost.
...
## Post-review #1 â€” chore: lifespan, docs gating, doc fixes

Commit `3651924`. Four cleanups bundled:

- `Base.metadata.create_all` moved into a FastAPI lifespan context. The original module-level call wrote `qr_code.db` to disk every time `app.main` was imported â€” including from pytest, which doesn't need it (the test fixture builds its own in-memory engine).
...
- DECISIONS.md Stage 5 corrected: 22 â†’ 25 tests.

## Post-review #2 â€” feat(security): SSRF and CRLF hardening in url_validator
...
2. **Internal hostnames blocked by name.** A name like `localhost` or `metadata.google.internal` only resolves to its dangerous IP at fetch time, after our validator has run. The `BLOCKED_HOSTNAMES` set rejects the common ones by string match. This isn't a complete defense â€” an attacker can register a public DNS name that resolves to `127.0.0.1` (DNS rebinding) and bypass the static check â€” but it closes the trivial pathway and matches what production URL shorteners do at the validation stage.
...
- IDN / punycode homograph: `https://xn--vil-7ka.com` (visually "Ðµvil.com") still passes. Needs IDNA decoding before blocklist comparison.
...
## Post-review #3 â€” feat(auth): edit_token bearer on PATCH/DELETE
...
- `POST /api/qr/create` returns `edit_token: str` â€” 32 bytes from `secrets.token_urlsafe`, ~256 bits of entropy.
...
- **SHA-256 of a high-entropy plaintext, not bcrypt/argon2.** Password hashing functions (`bcrypt`, `argon2`) deliberately consume CPU to slow down brute-force against low-entropy user-chosen passwords. Our `edit_token` has ~256 bits of CSPRNG entropy from the start â€” brute-forcing it is computationally infeasible regardless of hash speed. SHA-256 is the right tool: it's fast (don't tax legit PATCH/DELETE requests), it's collision-resistant for our purposes, and the constant-time `hmac.compare_digest` removes timing-side-channel leakage.
- **Token returned exactly once, on create.** Not stored in plaintext server-side, not retrievable via any read endpoint. If the caller loses it, the short link is no longer editable â€” this matches the principle of least disclosure (we shouldn't be a custodian of the editing credential).
...
- Brute-forcing the bearer is still possible in principle; the rate limit only covers `POST /api/qr/create`. Adding a limit to `PATCH`/`DELETE` (e.g. `30/minute/IP`) would close this â€” flagged in the next review pass.
...
Memory management for the dedup dict: lazy GC inside `_record_scan` itself â€” when the dict crosses 5000 entries, we sweep entries older than 30Ã— the window. Avoids a background thread; cost is paid only when the dict grows large.

**Semantic note for `total_scans`:**

In production (`SCAN_DEDUP_WINDOW = 1.0`), `total_scans` now means "unique scans across (token, ip) per second-window," not "raw HTTP hits." For most use cases this is the more meaningful metric â€” a single user refreshing 50 times shouldn't read as 50 scans. Apps that need raw counts can set `SCAN_DEDUP_WINDOW = 0` via env-var-driven config (not wired up yet; flagged below).

**Bonus fix landing here (caught by security review Â§6):** `_record_scan` was storing `user_agent` raw into a `String(500)` column. SQLite ignores the length constraint, so a multi-megabyte UA string would be stored verbatim â€” storage-amplification primitive when chained with the unbounded redirect. We now truncate to 500 chars at insert time.

**Tests:**

- `test_redirect_rate_limit_fires_at_threshold` â€” set limit to `5/minute`, 6th redirect must 429.
- `test_scan_dedup_skips_rapid_scans_from_same_ip` â€” 5 rapid scans â†’ `total_scans == 1`.
- `test_scan_dedup_records_across_different_ips` â€” asserts the dict bookkeeping; cross-IP via TestClient is limited.
